...
if ($filepath)
{
    ...
    //获取视频信息
    $vodRes = $Conn ->Table('vod_zmy')-> Where('isuse=1')->Order('vod_id asc')->Select();
    ...
    if(false === empty($res['article_id'])){
        unlink($filepath);
        $res = $Conn ->Table('vod_zmy')-> Where('vod_id='.$updateId)->Edit(['isuse'=>2]);
    }

    //执行下一次下载
    $vodRes = $Conn ->Table('vod_zmy')-> Where('isuse=3')->Order('vod_id asc')->Select();
    if (true === empty($vodRes['url'])) {
        $vodRes = $Conn ->Table('vod_zmy')-> Where('isuse=0')->Order('vod_id asc')->Select();
        if (false === empty($vodRes['url'])) {

            $res = $Conn ->Table('vod_zmy')-> Where('vod_id='.$vodRes['vod_id'])->Edit(['isuse'=>3]);
            ...
            $res = $Conn ->Table('vod_zmy')-> Where('vod_id='.$vodRes['vod_id'])->Edit(['isuse'=>1]);
            phpLog('下载视频完成');
            phpLog($res);
        }
    }
